...
export function StoreBrandProvider({ children }: { children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    if (cached?.theme) applyStoreTheme({ ...defaultTheme, ...cached.theme });
  }, []);
  ...
  useEffect(() => {
    storeApi.info()
      ...
      .catch(() => {
        const fallbackCached = loadCachedStoreBrand();
        if (fallbackCached?.theme) applyStoreTheme({ ...defaultTheme, ...fallbackCached.theme });
